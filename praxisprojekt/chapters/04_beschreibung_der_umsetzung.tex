...
Architektur war eine strikte Trennung zwischen Datenhaltung und Präsentation im Sinne der \textit{Separation of Concerns}, um eine nachhaltige Wartbarkeit zu gewährleisten 
\cite{Martin2017}. Während die Applikation die Logik zum Rendern der Spielwelt enthält, 
...
\begin{itemize}
    \item \textit{world \& scene:} Diese Tabellen bilden die topografische Hierarchie ab. Analog zu Videospiel-Klassikern wie „Super Mario Bros.“ ist das System auf Skalierbarkeit 
    ...
    vorgeben.
    \item \textit{learning\_units:} Hier werden die eigentlichen didaktischen Einheiten verwaltet. Durch die Verknüpfung mit Szenen signalisiert die API der App, welche Lerninhalte 
    an welchem Ort der grafischen Spielwelt aktiv sind.
    \item \textit{learning\_unit\_requirement:} Zur Steuerung des Lernpfads wurde eine Abhängigkeitslogik implementiert. Diese Tabelle definiert Voraussetzungen (Prerequisites), sodass 
    bestimmte Einheiten erst nach dem erfolgreichen Abschluss vorangegangener Aufgaben freigeschaltet werden.
    \item \textit{content:} Um eine maximale Flexibilität zu gewährleisten, wurde die Content-Tabelle hochgradig abstrakt entworfen. Sie speichert unterschiedliche Datentypen. Von URLs 
    ...
    \item \textit{user\_progress:} Diese Tabelle persisitiert den individuellen Fortschritt der Nutzer. Sie bildet die Grundlage für das serverseitige Belohnungssystem und stellt sicher,
...
\textit{Firebase} \cite{Firebase} realisiert. Zur Absicherung der Kommunikation wird bei jedem API-Aufruf ein von Firebase generierter User-Token im Header mitgeschickt. Dieser wird serverseitig 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/RuntimeDiagramLogin.png}
...
\textit{Vue.js} in Kombination mit dem Build-Tool \textit{Vite}. Um eine hohe Typsicherheit und Wartbarkeit innerhalb der heterogenen Datenstrukturen des Projekts zu gewährleisten, 
wurde die gesamte Geschäftslogik konsequent in \textit{TypeScript} umgesetzt.
...
performante Prüfung des Freischaltstatus innerhalb der App-Oberfläche. Durch die Einbindung der \textit{EndpointGroups} wird zudem sichergestellt, dass die API nur die für den 
...
\begin{itemize}
    \item \textit{Opening:} Einstieg in die Thematik.
    \item \textit{Problem:} Visualisierung der Risiken.
    \item \textit{Learning:} Vermittlung von Fachwissen durch Learning Nuggets.
    \item \textit{Action:} Durchführung einer interaktiven Challenge.
    \item \textit{Reward:} Abschluss und Belohnung.
\end{itemize}
...
\begin{itemize}
    \item \textit{SlideshowScene:} Dieser Typ wird für die narrativen Phasen (\textit{Opening, Problem, Learning, Reward}) verwendet. Da diese primär aus Comic-Szenen 
    ...
    \item \textit{ActionScene:} Diese Szene bildet den interaktiven Kern (Phase \textit{Action}). Sie transformiert die JSON-basierten Challenge-Daten der API automatisch in eine 
    Multiple-Choice-Aufgabe.
    \item \textit{RoadmapScene:} Unabhängig von den Lerneinheiten dient dieser Typ zur Darstellung der Weltkarte und ermöglicht den Einstieg in die freigeschalteten Einheiten.
\end{itemize}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/ClassDiagram_Entity.png}
    \caption{Entity-System und Spezialisierung der generischen Szenentypen}
    \label{fig:engine_entities}
\end{figure}

Die Darstellung innerhalb der Engine erfolgt über ein modular aufgebautes \textit{Entity-System}. Jedes grafische Element ist eine Instanz einer \texttt{Entity}-Klasse 
mit eigenem Lebenszyklus, welcher primär durch die Methoden \texttt{update} und \texttt{render} definiert wird. 

Ein zentraler Aspekt der Architektur ist die Anwendung des \textit{Composite Patterns}: Durch die strukturelle Trennung in \texttt{Component} und \texttt{Entity} können 
komplexe Objekte aus wiederverwendbaren Verhaltensbausteinen komponiert werden. Die visuelle Repräsentation wird dabei konsequent über ein dediziertes 
\texttt{Style}-System gesteuert, welches die Parameter für die \texttt{render}-Methoden liefert und so eine saubere Trennung zwischen Logik und Aussehen erzwingt. Zur 
Verwaltung dynamischer Zustände verfügt jede Entity über ein internes \texttt{State}-Management, das den aktuellen Status der Einheit persistiert und Zustandsübergänge 
innerhalb des Spielzyklus ermöglicht.

In der Umsetzung wurde verstärkt auf Konzepte der objektorientierten Programmierung (OOP) gesetzt. So werden durch den Einsatz von \textit{Interfaces} und 
\textit{abstrakten Klassen} klare Kontrakte für das System definiert. Ein Beispiel hierfür ist das \texttt{InteractiveEntity}, welches als Abstraktionsschicht für alle 
Objekte dient, die auf Nutzerinteraktionen reagieren können.

Beim Abschluss der finalen \texttt{reward}-Szene wird über ein Callback-System die Methode \texttt{updateCompletedStatus} im Backend aufgerufen, wodurch der Fortschritt 
persistent gespeichert und die Navigation zurück zur Roadmap eingeleitet wird.
